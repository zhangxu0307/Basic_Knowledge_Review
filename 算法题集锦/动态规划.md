...
其中dp[i][j]表示到达(i, j)位置所能组成的最大正方形的边长
...
dp[i]记录的是到达i花费的cost是多少，注意，这里到达i，并没有加入i位置的cost，只是表示到i位置之前花费的cost。由于在数组末尾会跳出数组一步（最后可以跳两步），因此dp数组要开成一个大小为n的。
